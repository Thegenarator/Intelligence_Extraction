...
## Agent Behavior (for Hackathon Evaluation)

- **Persona**:
  - Polite, cautious, slightly slow/naïve adult
  - Keeps replies short (2–4 sentences)
  - Asks exactly one concrete question per turn
- **Phases**:
  - **HOOK**: show interest, ask what’s needed to proceed.
  - **PROBE**: clarify workflow, payment channel, and amount.
  - **HARVEST**: explicitly but plausibly ask for UPI ID, account+IFSC, or URL (one at a time).
  - **NOT_SCAM**: neutral, non-committal; no harvesting.
- **Guardrails**:
  - Never mention “scam”, “security checks”, “AI”, or internal reasoning.
  - Hard cap on turns per conversation (`MAX_TURNS`).
  - `SCAM_THRESHOLD` and `HARVEST_HINT_THRESHOLD` configurable via env.

---

## Deployment (for the hackathon)

1) Deploy this FastAPI app to your preferred platform (Render, Fly.io, Railway, Cloud Run, etc.).
2) Expose `POST /webhook` publicly over HTTPS.
3) Configure the **same `API_KEY`** in your cloud environment.
4) Provide the hackathon with:
   - **Webhook URL**: e.g. `https://your-app.onrender.com/webhook`
   - **Header**: `X-API-Key: <your-API_KEY>`

The Mock Scammer API will then:
- Send scam messages to `/webhook`
- Receive your structured JSON (detection, reply, extracted intel)
- Score you on detection accuracy, engagement quality, and intel extraction.*** End Patch```} ***!
